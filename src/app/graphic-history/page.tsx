...
export default function GraphicHistoryPage() {
            ...
            <DashboardLayout>
                <div className="min-h-screen bg-slate-950 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-slate-400">Đang tải lịch sử Graphic Design...</p>
                    </div>
                </div>
            </DashboardLayout>
        )
                ...
                <main className="p-6">
                    <div className="flex flex-wrap items-center gap-4 mb-6 bg-slate-800/30 p-4 rounded-xl border border-slate-700/50">
                        <div className="relative flex-1 min-w-[200px]">
                            ...
                            <input
                                type="text"
                                placeholder="Tìm theo tên task hoặc designer..."
                                value={searchQuery}
                    ...
                    {filteredChanges.length === 0 ? (
                        <div className="text-center py-20 bg-slate-800/30 rounded-2xl border border-slate-700/50">
                            ...
                            <p className="text-sm text-slate-500 mt-2">
                                Khi due date của task design bị thay đổi, lịch sử sẽ được ghi lại tại đây
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <thead className="bg-slate-700/30">
                                        <tr>
                                            ...
                                            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                                <div className="flex items-center gap-1"><User className="w-3.5 h-3.5" />Designer</div>
                                            </th>
                                            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Due Date cũ</th>
                                            <th className="text-center px-1 py-3 text-xs font-semibold text-slate-400"></th>
                                            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Due Date mới</th>
                                            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Thay đổi bởi</th>
                                            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                                <div className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />Thời gian</div>
                                            </th>
                                        </tr>
                                    </thead>
                                            ...
                                            return (
                                                <tr key={change.id} className="hover:bg-slate-700/20 transition-colors">
                                                    <td className="px-4 py-3 text-sm text-white max-w-[300px]">
                                                        <div className="truncate" title={change.task_name}>{change.task_name}</div>
                                                    </td>
                                                    <td className="px-4 py-3 text-sm text-slate-300">{change.assignee_name || '-'}</td>
                                                    <td className="px-4 py-3 text-center">
                                                        <span className="inline-block px-2 py-1 bg-slate-700/50 rounded-lg text-sm text-slate-300">{formatDate(change.old_due_date)}</span>
                                                    </td>
                                                    <td className="px-1 py-3 text-center">
                                                        <ArrowRight className={`w-4 h-4 mx-auto ${arrow}`} />
                                                    </td>
                                                    <td className="px-4 py-3 text-center">
                                                        <span className={`inline-block px-2 py-1 rounded-lg text-sm font-medium ${arrow === 'text-red-400' ? 'bg-red-500/20 text-red-300' : arrow === 'text-green-400' ? 'bg-green-500/20 text-green-300' : 'bg-slate-700/50 text-slate-300'}`}>
                                                            {formatDate(change.new_due_date)}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3 text-sm text-cyan-300">{change.changed_by}</td>
                                                    <td className="px-4 py-3 text-sm text-slate-500 whitespace-nowrap">{formatDateTime(change.changed_at)}</td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
